...
import { AlertCircle, ArrowLeft, Download, Loader, Palette, Save, Trash2 } from 'lucide-react'
import StepProgress from './StepProgress'
import Modal from './Modal'
//rresizer observer hook
...
const EditResume = () => {
  ...
  //update array items state usingindex
  ...
  //fetching resume details backend url
  ...
  //it will help in choosing the preview and downloading the the resume also save the resume as image
  ...
  try {
    ...
    // 🔥 Override ALL colors to safe values
    const override = document.createElement("style");
    override.id = "__thumbnail_override__";
    ...
    document.getElementById("__thumbnail_override__")?.remove();
  ...
  const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      console.log("Update URL:", API_PATHS.RESUME.UPDATE(resumeId));
  ...
  //   }
  // },[])
  // delete function to dleete any resume
    ...
    <div className="w-full max-w-[800px] flex flex-col items-start">
      ...
      <Modal isOpen={openPreviewModal} onClose={() => setOpenPreviewModal(false)}
        ...
        actionBtnText={isDownloading ? "Generating..." :
          downloadSuccess ? "Downloaded!" : "Dounload pdf"
        }
        actionBtnIcon={
          isDownloading?(
